...
public class ArticleLikeServiceImpl implements ArticleLikeService {
    ...
    public ArticleLikeVo likeStatus(Long articleId) {
        ArticleLike articleLike = articleLikeMapper.getLikeByArticleId(articleId,SecurityUtils.getUserId());
        if(articleLike==null){
            //点赞表中没有记录，说明未点赞
    ...
    public ArticleLikeVo likeArticle(Long articleId) {
        ...
        if(!user.getId().equals(article.getCreateBy())){
            //查询通知表中是否有记录
            CommentNotification success = commentNotificationMapper.getByByArticleAndFromUser(articleId,SecurityUtils.getUserId(),NotificationType.ARTICLE_LIKE);
            if(success!=null){//存在的话，直接设置del为0
                commentNotificationMapper.setDelTo0ByArticleAndFromUser(articleId,SecurityUtils.getUserId(),NotificationType.ARTICLE_LIKE);
            }else {
                CommentNotification commentNotification = new CommentNotification();
                commentNotification.setType(NotificationType.ARTICLE_LIKE);
                commentNotification.setTitle("文章新点赞通知");
                commentNotification.setContent("用户"+user.getNickName()+"点赞了你的"+article.getTitle()+"文章");
    ...
    public ArticleLikeVo unlikeArticle(Long articleId) {
        ...
        articleMapper.updateArticle(article);
        //如果取消点赞，需要把通知点赞的给删除
        commentNotificationMapper.setDelTo1ByArticleAndFromUser(articleId,SecurityUtils.getUserId(),NotificationType.ARTICLE_LIKE);
        //点赞表删除记录
